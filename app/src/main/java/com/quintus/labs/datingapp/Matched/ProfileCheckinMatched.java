...
public class ProfileCheckinMatched extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CalculateAge cal = new CalculateAge(user.getNascimento());
        int age = cal.getAge();
        ...
        String fixErrorBuildbyRamon = "1234";
        if (fixErrorBuildbyRamon.length() != 0) {
            profile_bio.setText(fixErrorBuildbyRamon);
        }

        if (fixErrorBuildbyRamon.length() != 0) {
            profile_numbers.setText(user.getTelefone());
        } else {
        ...
        String profileImageUrl = fixErrorBuildbyRamon;
        ...
        sendSMSButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                sendSMS(user.getTelefone(), user.getUsername());
            }
        });
